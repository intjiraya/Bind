...
public class GetPlayerByDiscordIdUseCase(IPlayerRepository repository) : IGetPlayerByDiscordIdUseCase
{
    public async Task<ErrorOr<PlayerResponse>> ExecuteAsync(DiscordId discordId, CancellationToken ct)
    {
        var player = await repository.GetByDiscordIdAsync(discordId, ct);
        if (player is null)
